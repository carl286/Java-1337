...
public class TicTacToe {
    ...
    public int move(int row, int col, int player) {
        //as all moves are valid, this saves us lots of troubles.
        //place the pos
        ++rows[player-1][row];
        ++cols[player-1][col];
        if (row == col)
            ...
            ++antiDiags[player-1];
        if (rows[player-1][row] == size || cols[player-1][col] == size || diags[player-1] == size || antiDiags[player-1] == size)
            return player;
        return 0;
    }
    /**
