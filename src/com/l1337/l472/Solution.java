...
public class Solution {
    ...
    //trie is not null, s is not empty
    private void addToTrie(Trie trie, String s) {
        int depth = 0;
        while (depth < s.length()) {
            if (trie.array[s.charAt(depth) - 'a'] == null)
                trie.array[s.charAt(depth) - 'a'] = new Trie();
            trie = trie.array[s.charAt(depth) - 'a'];
            ++depth;
        }
        trie.ended = true;
    }

    private boolean bulkSearch(Trie root, String word, int depth) {
        Trie trie = root;
        for (int i = depth; i < word.length(); i++) {
            if (trie.array[word.charAt(i) - 'a'] != null) {
                trie = trie.array[word.charAt(i) - 'a'];
                if (trie.ended && (i + 1 >= word.length() || bulkSearch(root, word, 1 + i))) {
                    return true;
                }
            } else {
                return false;
...
//        }
        ...
        for (String word : words) {
            ...
            for (int k = 0; k + 1 < word.length(); ++k) {
                if (trie.array[word.charAt(k) - 'a'] != null) {
                    trie = trie.array[word.charAt(k) - 'a'];
                    if (trie.ended && bulkSearch(root, word, 1 + k)) {
                        ret.add(word);
                        break;
                    }
                } else {
